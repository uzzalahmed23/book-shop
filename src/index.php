<?php
namespace Agency\Widgets;

use \Elementor\Widget_Base;

class LatestPosts extends Widget_Base
{
    public function get_name()
    {
        return 'agency_latest_posts';
    }

    public function get_title()
    {
        return __('Latest Posts', 'agency');
    }

    public function get_icon()
    {
        return 'eicon-price-table';
    }

    public function get_categories()
    {
        return ['agency'];
    }

    public function get_style_depends()
    {
        return ['agency-widget-latest-posts'];
    }

    protected function register_controls()
    {

    }

    protected function render()
    {
        $args = array(
            'post_type' => 'post',
            'post_status' => 'publish',
            'posts_per_page' => 3,
        );
        $latest_posts_query = new \WP_Query($args);

        ?>
        <?php if ($latest_posts_query->have_posts()): ?>
            <div class="agency-latest-posts">
                <div class="agency-latest-posts__grid">
                    <?php while ($latest_posts_query->have_posts()): $latest_posts_query->the_post(); ?>
                    <article class="__post">
                        <div class="__media">
                            <?php the_post_thumbnail( 'blog_medium' ); ?>
                        </div>
                        <div class="__content">
                            <div class="__meta">
                                <a class="__meta-item">
                                    <span class="__icon">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M14.8386 12.3647C14.9355 12.4223 15.0552 12.4899 15.1908 12.5664C15.7848 12.9015 16.6827 13.4081 17.2979 14.0102C17.6826 14.3868 18.0482 14.883 18.1146 15.491C18.1852 16.1375 17.9032 16.7443 17.3373 17.2833C16.3612 18.2133 15.1897 18.9587 13.6744 18.9587H6.32587C4.81062 18.9587 3.63912 18.2133 2.66292 17.2833C2.09707 16.7443 1.81502 16.1375 1.88569 15.491C1.95214 14.883 2.31769 14.3868 2.70242 14.0102C3.31755 13.4081 4.21547 12.9015 4.80949 12.5664C4.94503 12.4899 5.06477 12.4223 5.1617 12.3647C8.12341 10.6011 11.8769 10.6011 14.8386 12.3647Z" fill="#090B0E"></path><path d="M5.625 5.41699C5.625 3.00074 7.58375 1.04199 10 1.04199C12.4163 1.04199 14.375 3.00074 14.375 5.41699C14.375 7.83323 12.4163 9.79199 10 9.79199C7.58375 9.79199 5.625 7.83323 5.625 5.41699Z" fill="#090B0E"></path></svg>
                                    </span>
                                    <span class="__text"><?php echo get_the_author(); ?></span>
                                </a>
                                <a class="__meta-item">
                                    <span class="__icon">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none"><path d="M5.83366 1.66634C5.83366 1.20611 5.46056 0.833008 5.00033 0.833008C4.54009 0.833008 4.16699 1.20611 4.16699 1.66634V2.04038C4.48393 1.93985 4.82161 1.86807 5.17977 1.81602C5.38639 1.78598 5.60426 1.76197 5.83366 1.74278V1.66634Z" fill="#090B0E"></path><path d="M15.8337 2.04038C15.5167 1.93985 15.1791 1.86807 14.8209 1.81602C14.6142 1.78598 14.3964 1.76197 14.167 1.74278V1.66634C14.167 1.20611 14.5401 0.833008 15.0003 0.833008C15.4606 0.833008 15.8337 1.20611 15.8337 1.66634V2.04038Z" fill="#090B0E"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M10.8573 1.66699H9.14267C7.51912 1.66697 6.20951 1.66694 5.17944 1.81667C4.10446 1.97291 3.214 2.30683 2.51502 3.06249C1.82458 3.80892 1.52733 4.74426 1.3868 5.87421C1.24998 6.97444 1.24998 8.37874 1.25 10.1452V10.6887C1.24998 12.4552 1.24998 13.8596 1.3868 14.9597C1.52733 16.0897 1.82458 17.0251 2.51502 17.7715C3.214 18.5272 4.10446 18.8611 5.17944 19.0173C6.20952 19.1671 7.51911 19.167 9.14267 19.167H10.8573C12.4809 19.167 13.7905 19.1671 14.8206 19.0173C15.8956 18.8611 16.786 18.5272 17.485 17.7715C18.1754 17.0251 18.4727 16.0897 18.6132 14.9597C18.75 13.8596 18.75 12.4552 18.75 10.6887V10.1452C18.75 8.37874 18.75 6.97445 18.6132 5.87421C18.4727 4.74426 18.1754 3.80892 17.485 3.06249C16.786 2.30683 15.8956 1.97291 14.8206 1.81667C13.7905 1.66694 12.4809 1.66697 10.8573 1.66699ZM3.74819 7.50033C3.36601 7.50033 3.17492 7.50033 3.05321 7.62047C2.93151 7.7406 2.92906 7.92926 2.92417 8.30658C2.91689 8.86758 2.91667 9.49549 2.91667 10.203V10.631C2.91667 12.4683 2.91818 13.7687 3.04073 14.7541C3.16106 15.7217 3.38553 16.2582 3.73852 16.6397C4.08297 17.0122 4.55392 17.2422 5.41917 17.368C6.31548 17.4982 7.50354 17.5003 9.20833 17.5003H10.7917C12.4964 17.5003 13.6845 17.4982 14.5808 17.368C15.4461 17.2422 15.917 17.0122 16.2615 16.6397C16.6145 16.2582 16.8389 15.7217 16.9593 14.7541C17.0818 13.7687 17.0833 12.4683 17.0833 10.631V10.203C17.0833 9.49549 17.0831 8.86758 17.0758 8.30658C17.0709 7.92926 17.0685 7.7406 16.9468 7.62047C16.8251 7.50033 16.634 7.50033 16.2518 7.50033H3.74819Z" fill="#090B0E"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M5.625 10.8337C5.625 10.2583 6.09137 9.79199 6.66667 9.79199H6.67414C7.24944 9.79199 7.71581 10.2583 7.71581 10.8337C7.71581 11.409 7.24944 11.8753 6.67414 11.8753H6.66667C6.09137 11.8753 5.625 11.409 5.625 10.8337ZM8.95458 10.8337C8.95458 10.2583 9.421 9.79199 9.99625 9.79199H10.0038C10.579 9.79199 11.0454 10.2583 11.0454 10.8337C11.0454 11.409 10.579 11.8753 10.0038 11.8753H9.99625C9.421 11.8753 8.95458 11.409 8.95458 10.8337ZM12.2842 10.8337C12.2842 10.2583 12.7506 9.79199 13.3258 9.79199H13.3333C13.9087 9.79199 14.375 10.2583 14.375 10.8337C14.375 11.409 13.9087 11.8753 13.3333 11.8753H13.3258C12.7506 11.8753 12.2842 11.409 12.2842 10.8337ZM5.625 14.167C5.625 13.5917 6.09137 13.1253 6.66667 13.1253H6.67414C7.24944 13.1253 7.71581 13.5917 7.71581 14.167C7.71581 14.7423 7.24944 15.2087 6.67414 15.2087H6.66667C6.09137 15.2087 5.625 14.7423 5.625 14.167ZM8.95458 14.167C8.95458 13.5917 9.421 13.1253 9.99625 13.1253H10.0038C10.579 13.1253 11.0454 13.5917 11.0454 14.167C11.0454 14.7423 10.579 15.2087 10.0038 15.2087H9.99625C9.421 15.2087 8.95458 14.7423 8.95458 14.167Z" fill="#090B0E"></path></svg>
                                    </span>
                                    <span class="__text"><?php echo get_the_date(); ?></span>
                                </a>
                            </div>
                            <h3 class="__title">
                                <a href="<?php echo get_the_permalink(); ?>"><?php echo get_the_title(); ?></a>
                            </h3>
                            <div class="__excerpt">
                                <?php the_excerpt(); ?>
                            </div>
                            <a href="<?php echo get_the_permalink(); ?>" class="__readmore">Read More <svg xmlns="http://www.w3.org/2000/svg" width="14" height="13" viewBox="0 0 14 13" fill="none"><path d="M1.27346 6.16697H13.0586M13.0586 6.16697L7.75527 0.86367M13.0586 6.16697L7.75527 11.4703" stroke="#090B0E" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg></a>
                        </div>
                    </article>
                    <?php endwhile; ?>
                </div>
                <?php wp_reset_postdata(); ?>
            </div>
        <?php endif; ?>

        <?php
    }
}
